...
export default function SeatOrderPage() {
  ...
  return (
    <main style={s.page}>
      ...
      {players.length < 2 && (
        <p style={s.warning}>Add at least 2 players to continue.</p>
      )}

      <button style={{ ...s.btn, opacity: players.length < 2 ? 0.4 : 1 }} onClick={handleNext} disabled={players.length < 2}>
        Next: Review & Start →
      </button>
    </main>
  );
...
const s: Record<string, React.CSSProperties> = {
  ...
  warning: { fontSize: '0.85rem', color: '#888', margin: 0 },
